...
internal static class PackHelper
{
    ...
    public static List<T> LoadSourceRead<T>(SiemensS7Master device, IEnumerable<IVariable> deviceVariables, int maxPack, string defaultIntervalTime) where T : IVariableSource, new()
    {
        ...
        {
            // 使用字典存储地址和对应的变量
            Dictionary<SiemensS7Address, IVariable> map = item.ToDictionary(it =>
            {
            ...
            foreach (var functionCode in functionCodes)
            {
                ...
                foreach (var stationNumber in dbNumbers)
                {
                    ...
                    var tempResult = LoadSourceRead<T>(addressList!, (byte)functionCode, item.Key, device);
    ...
    private static List<T> LoadSourceRead<T>(Dictionary<SiemensS7Address, IVariable> addressList, byte functionCode, string intervalTime, SiemensS7Master siemensS7Net) where T : IVariableSource, new()
    {
