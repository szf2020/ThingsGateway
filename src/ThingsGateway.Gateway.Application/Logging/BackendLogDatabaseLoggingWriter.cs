...
public class BackendLogDatabaseLoggingWriter : IDatabaseLoggingWriter
{
    ...
    public async Task WriteAsync(LogMessage logMsg, bool flush)
    {
        ...
        if (logMsg.LogLevel >= CustomLevel)
        {
            ...
            if (flush)
            {
                // 如果SqlOrm客户端未初始化，则进行初始化
                sqlClient ??= DbContext.GetDB<BackendLog>();

                // 异步执行入库操作
                await sqlClient.Insertable(_logQueues.ToListWithDequeue()).ExecuteCommandAsync().ConfigureAwait(false);
