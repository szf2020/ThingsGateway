...
internal sealed class ChannelService : BaseService<Channel>, IChannelService
{
    ...
    /// </summary>
    /// <returns>列表</returns>
    public async Task<List<Channel>> GetFromDBAsync(Expression<Func<Channel, bool>> expression = null, ISqlClient db = null)
    {
    ...
    private async Task<Func<ISqlQueryable<Channel>, ISqlQueryable<Channel>>> GetWhereQueryFunc(GatewayExportFilter exportFilter)
    {
        ...
        var dataScope = await GlobalData.SysUserService.GetCurrentUserDataScopeAsync().ConfigureAwait(false);
        var whereQuery = (ISqlQueryable<Channel> a) => a
