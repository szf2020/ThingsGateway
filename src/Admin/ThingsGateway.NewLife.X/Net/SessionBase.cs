...
public abstract class SessionBase : DisposeBase, ISocketClient, ITransport, ILogFeature
{
    ...
    {
        var sock = Client;
        if (sock?.Connected != true) return "Disconnected";
    ...
    private const Int32 _IntoThreadCount = 10;
    ...
    private void ProcessReceive(ArrayPacket pk, SocketAsyncEventArgs se, IPEndPoint remote)
    {
        // 打断上下文调用链，这里必须是起点
        ...
        try
        {
            ...
            if (LogReceive && Log?.Enable == true) WriteLog("Recv [{0}]: {1}", total, pk.ToHex(LogDataLength));
    ...
    public void WriteLog(String format, params Object?[] args)
    {
        ...
        if (Log?.Enable == true) Log.Info($"[{LogPrefix}]{format}", args);
    }
