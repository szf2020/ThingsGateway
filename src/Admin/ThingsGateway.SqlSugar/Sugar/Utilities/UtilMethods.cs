...
namespace ThingsGateway.SqlSugar
{
    public static class UtilMethods
    {
        ...
        {
            if (propertyType == UtilConstants.BoolType && item.DefaultValue?.EqualCase("true") == true)
            {
                item.DefaultValue = "1";
            }
            else if (propertyType == UtilConstants.BoolType && item.DefaultValue?.EqualCase("false") == true)
            {
                item.DefaultValue = "0";
            }
        }
        ...
        public static IEnumerable<T> BuildTree<T>(ISqlSugarClient db, IEnumerable<T> list, string idName, string pIdName, string childName, object rootValue)
        {
            ...
            var root = rootValue != null ? group.FirstOrDefault(x => x.Key == rootValue.ObjToString()) : group.FirstOrDefault(x => x.Key == null || string.IsNullOrEmpty(x.Key) || x.Key == "0" || x.Key == Guid.Empty.ToString());
        ...
        {
            if (targetType == null || interfaceType?.IsInterface != true)
            {
                return false;
            }
        ...
        public static object ConvertDataByTypeName(string ctypename, string value)
        {
            ...
            else if (item.CSharpTypeName.EqualCase("byte[]") && item.FieldValue?.Contains('|') == true)
            {
        ...
        public static string GetSqlString(ConnectionConfig connectionConfig, KeyValuePair<string, IReadOnlyCollection<SugarParameter>> sqlObj)
        {
            ...
            if (sqlObj.Value != null)
            {
                foreach (var item in UtilMethods.CopySugarParameters(sqlObj.Value).OrderByDescending(it => it.ParameterName.Length))
                {
                    ...
                    {
                        result = result.Replace(item.ParameterName, "0x" + Convert.ToHexString((byte[])item.Value));
                    }
        ...
        internal static object TimeOnlyToTimeSpan(object value)
        {
            ...
            return method.Invoke(value, Array.Empty<object>());
        }
