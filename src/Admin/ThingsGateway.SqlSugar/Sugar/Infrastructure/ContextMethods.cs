...
namespace SqlSugar
{
    public partial class ContextMethods : IContextMethods
    {
        ...
        public async Task<List<T>> DataReaderToSelectJsonListAsync<T>(IDataReader dataReader)
        {
            ...
            using (dataReader)
            {
                while (await ((DbDataReader)dataReader).ReadAsync().ConfigureAwait(false))
                {
                    var value = dataReader.GetValue(0);
                    if (value == null || value == DBNull.Value)
                    {
        ...
        private Dictionary<string, object> DataReaderToList<T>(IDataReader reader, Type tType, List<PropertyInfo> classProperties, List<T> reval)
        {
            ...
            foreach (var item in classProperties)
            {
                ...
                else
                {
                    ...
                    {
                        var addValue = readerValues.TryGetValue(name, out object? value) ? value : readerValues.First(it => it.Key.Equals(name, StringComparison.CurrentCultureIgnoreCase)).Value;
        ...
        private Dictionary<string, object> DataReaderToDynamicList_Part<T>(Dictionary<string, object> readerValues, PropertyInfo item, List<T> reval, Dictionary<string, string> mappingKeys = null, List<string> ignoreColumns = null)
        {
            ...
            foreach (var prop in classProperties)
            {
                ...
                else
                {
                    ...
                    var info = readerValues.Select(it => it.Key).FirstOrDefault(it => it.Equals(key, StringComparison.CurrentCultureIgnoreCase));
                    if (info == null)
                    {
                        key = item.Name + "." + name;
                    ...
                    var oldInfo = info;
                    if (mappingKeys != null && mappingKeys.TryGetValue(item.Name, out string? value))
                    {
                        key = value + "." + typeName + "." + name;
                        info = readerValues.Select(it => it.Key).FirstOrDefault(it => it.Equals(key, StringComparison.CurrentCultureIgnoreCase));
                        if (info == null)
                        {
                            var key2 = item.Name + "." + name;
                            info = readerValues.Select(it => it.Key).FirstOrDefault(it => it.Equals(key2, StringComparison.CurrentCultureIgnoreCase));
                        }
                    }
                    else if (mappingKeys?.ContainsKey("Single_" + name) == true)
                    {
