...
namespace ThingsGateway.SqlSugar
{
    public partial class SqlSugarClient : ISqlSugarClient, ITenant
    {
        ...
        public T CreateContext<T>(bool isTran = true) where T : SugarUnitOfWork, new()
        {
            ...
            var properies = ReflectionInoCacheService.Instance.GetOrCreate(cacheKey,
                ...
                (it.PropertyType.BaseType?.Name.StartsWith("SimpleClient`") == true)
                  ||
    ...
    where T4 : class, new()
        ...
        {
            _IsAllTran = true;
            await AllClientEachAsync(async it => await it.Ado.BeginTranAsync().ConfigureAwait(false)).ConfigureAwait(false);
        }

        public async Task BeginTranAsync(IsolationLevel iso)
        {
            _IsAllTran = true;
            await AllClientEachAsync(async it => await it.Ado.BeginTranAsync(iso).ConfigureAwait(false)).ConfigureAwait(false);
        }
        ...
        {
            return _ThreadId == Environment.CurrentManagedThreadId.ToString();
        }
        ...
        protected virtual void InitContext(ConnectionConfig config)
        {
            ...
            _ThreadId = Environment.CurrentManagedThreadId.ToString();
        ...
        private async Task AllClientEachAsync(Func<ISqlSugarClient, Task> action)
        {
            ...
            if (_AllClients.HasValue())
            {
                foreach (var item in _AllClients.Where(it => it.Context.HasValue()))
                {
                    await action(item.Context).ConfigureAwait(false);
                }
            }
        }
