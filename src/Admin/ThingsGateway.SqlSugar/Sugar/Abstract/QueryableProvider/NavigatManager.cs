    ...
    public class NavigatManager<T>
    {
        ...
        private void ExecuteByLay(Expression expression, List<object> list, Func<ISugarQueryable<object>, List<object>> selector)
        {
            ...
            else if (navObjectNameColumnInfo.Navigat.NavigatType == NavigateType.OneToManyByArrayList)
            {
                ...
                {
                    OneToManyByArrayList(pageList, selector, listItemEntity, navObjectNameProperty, navObjectNameColumnInfo);
                });
            }
                ...
                if (navList.HasValue())
                {
                    var groupQuery =
                               (from l in list.Distinct()
                                ...
                                where nValues?.Select(x => x.ObjToString()).Contains(lValue) == true
                                select new
                                {
                                    l,
                                    n
                    ...
                    foreach (var item in groupQuery)
                    {
                        ...
                        var itemSelectList1 = itemSelectList.ToList();
                        if (sqlObj.MappingExpressions.HasValue())
                        {
                            MappingFieldsHelper<T> helper = new MappingFieldsHelper<T>();
                            helper.NavEntity = navEntityInfo;
                            helper.Context = this.Context;
                            helper.RootEntity = this.Context.EntityMaintenance.GetEntityInfo<T>();
                            helper.SetChildList(navObjectNameColumnInfo, item.Key, itemSelectList1, sqlObj.MappingExpressions);
                        }
                        else
                        {
                            ...
                            var ilist = instance as IList;
                            foreach (var value in itemSelectList1)
                            {
                                ilist.Add(value);
                            }
        ...
        private void Select(string propertyName, SqlInfo result, MethodCallExpression method, ISugarQueryable<object> queryable)
        {
            ...
            if (types?.Length > 0)
            {
                ...
                else
                {
                    ...
                    if (string.IsNullOrEmpty(result.SelectString))
                    {
                ...
                if (propertyName != null)
                {
                    ...
                    if (pkColumnsInfo != null && fkColumnsInfo == null && result.SelectString?.Contains(queryable.SqlBuilder.GetTranslationColumnName(pkColumnsInfo.DbColumnName)) == false)
                    {
        ...
        private SqlSugarProvider GetCrossDatabase(SqlSugarProvider db, Type type)
        {
            ...
            else if (this.CrossQueryItems?.Count > 0 && this.CrossQueryItems.TryGetValue(type.FullName, out string? value))
            {
                var result = db.Root.GetConnection(value);
                return result;
            }
            else
            {
        ...
        private static void AppColumns(SqlInfo result, ISugarQueryable<object> queryable, string columnName)
        {
            ...
            if (result.SelectString?.Contains(selectPkName, StringComparison.CurrentCultureIgnoreCase) == false)
            {
                result.SelectString = result.SelectString + "," + (selectPkName + " AS " + selectPkName);
            }
        }
