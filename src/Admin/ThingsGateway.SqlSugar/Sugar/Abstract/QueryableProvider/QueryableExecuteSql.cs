...
namespace ThingsGateway.SqlSugar
{
    public partial class QueryableProvider<T> : QueryableAccessory, ISugarQueryable<T>
    {
        ...
        {
            var list = (await ToPivotEnumerableAsync(columnSelector, rowSelector, dataSelector).ConfigureAwait(false)).ToList();
            return this.Context.Utilities.SerializeObject(list);
        }
        #endregion
        ...
        public List<T> ToParentList(Expression<Func<T, object>> parentIdExpression, object primaryKeyValue)
        {
            ...
            if (current != null)
            {
                ...
                {
                    if (i > 200) { throw new SqlSugarException(ErrorMessage.GetThrowMessage("Dead cycle", "出现死循环或超出循环上限（200），检查最顶层的ParentId是否是null或者0")); }
