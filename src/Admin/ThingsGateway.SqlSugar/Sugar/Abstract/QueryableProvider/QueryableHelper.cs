...
namespace ThingsGateway.SqlSugar
{
    public partial class QueryableProvider<T> : QueryableAccessory, ISugarQueryable<T>
    {
        ...
        private async Task<List<T>> _ToParentListByTreeKeyAsync(Expression<Func<T, object>> parentIdExpression, object primaryKeyValue)
        {
            ...
            var current = await Context.Queryable<T>().AS(tableName).Filter(null, QueryBuilder.IsDisabledGobalFilter).Where(new List<IConditionalModel>() {
                new ConditionalModel()
                {
                    ConditionalType = ConditionalType.Equal,
                    CSharpTypeName = treeKey.PropertyInfo.PropertyType.Name,
            ...
            if (current != null)
            {
                ...
                object parentId = ParentInfo.PropertyInfo.GetValue(current, null);
                int i = 0;
                while (parentId != null && await Context.Queryable<T>().AS(tableName).Filter(null, QueryBuilder.IsDisabledGobalFilter).Where(new List<IConditionalModel>() {
                new ConditionalModel()
                {
                    ConditionalType = ConditionalType.Equal,
                    CSharpTypeName = treeKey.PropertyInfo.PropertyType.Name,
        ...
        private async Task<List<T>> _ToParentListByTreeKeyAsync(Expression<Func<T, object>> parentIdExpression, object primaryKeyValue, Expression<Func<T, bool>> parentWhereExpression)
        {
            ...
            var current = await Context.Queryable<T>().AS(tableName).WhereIF(parentWhereExpression != default, parentWhereExpression).Filter(null, QueryBuilder.IsDisabledGobalFilter).Where(new List<IConditionalModel>() {
                new ConditionalModel()
                {
                    ConditionalType = ConditionalType.Equal,
                    CSharpTypeName = treeKey.PropertyInfo.PropertyType.Name,
            ...
            if (current != null)
            {
                ...
                object parentId = ParentInfo.PropertyInfo.GetValue(current, null);
                int i = 0;
                while (parentId != null && await Context.Queryable<T>().AS(tableName).WhereIF(parentWhereExpression != default, parentWhereExpression).Filter(null, QueryBuilder.IsDisabledGobalFilter).Where(new List<IConditionalModel>() {
                new ConditionalModel()
                {
                    ConditionalType = ConditionalType.Equal,
                    CSharpTypeName = treeKey.PropertyInfo.PropertyType.Name,
        ...
        private void _CountEnd(MappingTableList expMapping)
        {
            ...
            if (expMapping.Count > 0)
            {
                ...
                this.QueryableMappingTableList.Add(expMapping[0]);
            }
        }
        ...
        protected async Task<TResult> _MinAsync<TResult>(Expression expression)
        {
            ...
            var isSingle = QueryBuilder.IsSingle();
            var lamResult = QueryBuilder.GetExpressionValue(expression, isSingle ? ResolveExpressType.FieldSingle : ResolveExpressType.FieldMultiple);
            var result = await MinAsync<TResult>(lamResult.GetResultString()).ConfigureAwait(false);
            QueryBuilder.SelectValue = null;
            return result;
        }
        ...
        protected async Task<TResult> _AvgAsync<TResult>(Expression expression)
        {
            ...
            var isSingle = QueryBuilder.IsSingle();
            var lamResult = QueryBuilder.GetExpressionValue(expression, isSingle ? ResolveExpressType.FieldSingle : ResolveExpressType.FieldMultiple);
            return await AvgAsync<TResult>(lamResult.GetResultString()).ConfigureAwait(false);
        }
        ...
        private async Task _InitNavigatAsync<TResult>(List<TResult> result)
        {
            ...
            {
                await Task.Run(() => _InitNavigat(result)).ConfigureAwait(false);
            }
        }
        private void _InitNavigat<TResult>(List<TResult> result)
        {
            if (this.QueryBuilder.Includes != null)
            {
                ...
                if (this.QueryBuilder.AppendNavInfo?.AppendProperties?.Count > 0)
                {
                    if (result.HasValue())
                    {
            ...
            if (result.Count != 0)
            {
                ...
                foreach (var it in managers)
                {
                    ...
                    var columns = allColumns
                        .Where(a => this.QueryBuilder.AppendNavInfo.Result[0].result.ContainsKey("SugarNav_" + a.PropertyName))
                        .ToList();
        ...
        private QueryableProvider<T> _Mapper<TObject>(Expression mapperObject, Expression mapperField)
        {
        ...
        private QueryableProvider<T> _Mapper<TObject>(Expression mapperObject, Expression mainField, Expression childField)
        {
            if ((mapperObject as LambdaExpression).Body is UnaryExpression)
            {
                mapperObject = ((mapperObject as LambdaExpression).Body as UnaryExpression).Operand;
        ...
        private string AppendSelectWithSubQuery(List<EntityColumnInfo> entityColumnInfos, string sql, ReadOnlyCollection<ParameterExpression> parameters, List<EntityColumnInfo> columnsResult, int parameterIndex1)
        {
            ...
            foreach (var item in columns)
            {
                if (item.IsIgnore == false && !this.QueryBuilder.AutoAppendedColumns.Contains(item.PropertyName) && columnsResult.Any(it => it.PropertyName.EqualCase(item.PropertyName)) && !list.Any(it => it.EqualCase(item.PropertyName)))
                {
                    ...
                    {
                        if (parameterIndex1 == 0 && this.QueryBuilder.JoinQueryInfos.Count != 0)
                        {
        ...
        public virtual JoinQueryInfo GetJoinInfo(Expression joinExpression, JoinType joinType)
        {
            ...
            {
                var firstPareamter = (express as LambdaExpression).Parameters[0];
                this.QueryBuilder.TableShortName = firstPareamter.Name;
                if (this.QueryBuilder.AsTables?.Count == 1)
                {
        ...
        {
            if (this.QueryBuilder.AsTables?.Any(it => it.Key == entityName) == true)
            {
        ...
        protected List<TResult> _ToList<TResult>()
        {
            ...
            if (IsCache)
            {
                ...
                result = CacheSchemeMain.GetOrCreate<List<TResult>>(cacheService, this.QueryBuilder, () => GetData<TResult>(sqlObj), CacheTime, this.Context, CacheKey);
            }
            else
            {
        ...
        protected async Task<List<TResult>> _ToListAsync<TResult>()
        {
            ...
            {
                result = await GetDataAsync<TResult>(sqlObj).ConfigureAwait(false);
            }
            RestoreMapping();
            await _InitNavigatAsync(result).ConfigureAwait(false);
            await _SubQueryAsync(result).ConfigureAwait(false);
            _Mapper(result);
            return result;
        }
        private void ToSqlBefore()
        {
            ...
            if (moreSetts?.IsWithNoLockQuery == true && string.IsNullOrEmpty(QueryBuilder.TableWithString))
            {
        ...
        protected async Task<List<TResult>> GetDataAsync<TResult>(KeyValuePair<string, IReadOnlyCollection<SugarParameter>> sqlObj)
        {
            ...
            if (entityType.IsInterface)
            {
                result = await GetDataAsync<TResult>(isComplexModel, QueryBuilder.AsType, dataReader).ConfigureAwait(false);
            }
            else
            {
                result = await GetDataAsync<TResult>(isComplexModel, entityType, dataReader).ConfigureAwait(false);
            }
            this.Db.GetDataAfter(sqlString, parameters);
            RestChangeMasterQuery(isChangeQueryableMasterSlave);
            RestChangeSlaveQuery(isChangeQueryableSlave);
        ...
        private async Task<List<TResult>> GetDataAsync<TResult>(bool isComplexModel, Type entityType, IDataReader dataReader)
        {
            ...
            if (entityType == UtilConstants.DynamicType)
            {
                result = await Context.Utilities.DataReaderToExpandoObjectListAsync(dataReader).ConfigureAwait(false) as List<TResult>;
            }
            else if (entityType.Name?.StartsWith("ValueTuple`") == true)
            {
                result = await Db.Context.Utilities.DataReaderToValueTupleTypeAsync<TResult>(dataReader).ConfigureAwait(false);
            }
            else if (entityType == UtilConstants.ObjType)
            {
            ...
            else if (entityType.IsAnonymousType() || isComplexModel || StaticConfig.EnableAot)
            {
                if (entityType.IsClass() == false && StaticConfig.EnableAot)
                {
                    result = await Bind.DataReaderToListAsync<TResult>(entityType, dataReader).ConfigureAwait(false);
                }
                else
                {
                    result = await Context.Utilities.DataReaderToListAsync<TResult>(dataReader).ConfigureAwait(false);
                    if (StaticConfig.EnableAot)
                        ResetNavigationPropertiesForAot(result);
                }
            }
        ...
        {
            if (cacheDurationInSeconds == int.MaxValue && this.Context.CurrentConnectionConfig.MoreSettings?.DefaultCacheDurationInSeconds > 0)
            {
        ...
        {
            return this.QueryBuilder.SubToListParameters?.Count > 0;
        }
        ...
        private void _SubQuery<TResult>(List<TResult> result)
        {
            ...
            var isSubToList = this.QueryBuilder.SubToListParameters != null && this.QueryBuilder.SubToListParameters.Count != 0;
            if (!isSubToList)
            {
                return;
            }
        ...
        private async Task _SubQueryAsync<TResult>(List<TResult> result)
        {
            ...
            await Task.Run(() => _SubQuery(result)).ConfigureAwait(false);
        }
        ...
        {
            if (this.QueryBuilder.IsSingle() == false && this.QueryBuilder.SelectValue is LambdaExpression exp && this.QueryBuilder.AsTables?.Count == 0)
            {
            ...
            return this.QueryBuilder.JoinIndex == 0 &&
                             this.QueryBuilder.IsSqlQuery == false &&
                               this.QueryBuilder.AsTables.Count == 0 &&
                                 this.QueryBuilder.IsSingle() &&
                                  (this.QueryBuilder.WhereInfos.Count != 0 || this.QueryBuilder.SelectValue is Expression);
        }
        #endregion
    }
}
