...
namespace ThingsGateway.SqlSugar
{
    public class UpdateBuilder : IDMLBuilder
    {
        ...
        protected virtual string ToSingleSqlString(List<IGrouping<int, DbColumnInfo>> groupList)
        {
                ...
                if (IsWhereColumns == false)
                {
                    ...
                    foreach (var item in PrimaryKeys)
                    {
                        ...
                        else
                        {
                            ...
                            var columnInfo = GetColumnInfo(item);
                            if (columnInfo?.SqlParameterDbType is System.Data.DbType valueDbType)
                            {
            ...
            if (this.JoinInfos != null && this.JoinInfos.Count != 0)
            {
        ...
        public virtual object FormatValue(object value)
        {
            ...
            else
            {
                ...
                {
                    string bytesString = "0x" + Convert.ToHexString((byte[])value);
                    return bytesString;
                }
                else if (type.IsEnum())
                {
        ...
        public virtual string GetDbColumn(DbColumnInfo columnInfo, object name)
        {
            ...
            else if (columnInfo.PropertyType?.Name == "DateOnly")
            {
        ...
        {
            return this.ReSetValueBySqlExpList?.ContainsKey(columnInfo.PropertyName) == true &&
                this.IsListUpdate == null &&
                DbColumnInfoList.GroupBy(it => it.TableId).Count() == 1;
        }
        private bool IsListSetExp(DbColumnInfo columnInfo)
        {
            return this.ReSetValueBySqlExpListType != null && this.ReSetValueBySqlExpList?.ContainsKey(columnInfo.PropertyName) == true;
        }
