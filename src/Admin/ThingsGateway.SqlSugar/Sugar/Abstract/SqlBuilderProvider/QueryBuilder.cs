...
namespace ThingsGateway.SqlSugar
{
    public abstract class QueryBuilder : IDMLBuilder
    {
        ...
        {
            var isSingle = Builder.QueryBuilder.JoinQueryInfos.IsNullOrEmpty() && EasyJoinInfos.Count == 0;
            return isSingle;
        }
        ...
        private void AppendTableFilter(SqlFilterItem item)
        {
            ...
            else if (isMain)
            {
                ...
                var isSameName = !isSingle && JoinQueryInfos.Any(it => it.TableName == entityInfo.DbTableName);
            ...
            else
            {
                ...
                if (easyInfo == null)
                {
                    if (ChildType.IsInterface && JoinQueryInfos.Any(it => it.EntityType?.GetInterfaces().Any(z => z == ChildType) == true))
                    {
        ...
        public virtual string ToJoinString(JoinQueryInfo joinInfo)
        {
            ...
            if (this.AsTables.Count != 0)
            {
                if (this.Context.MappingTables?.Any(it => it.DbTableName == name) == true)
                {
            ...
            var isSubQuery = name?.StartsWith('(') == true && name.EndsWith(')');
        ...
        public virtual string GetSelectValue
        {
            get
            {
                ...
                if (this.SubToListParameters != null && this.SubToListParameters.Count != 0)
                {
        ...
        public virtual string GetSelectValueByExpression()
        {
            ...
            string result = string.Empty;
            if (this.IgnoreColumns != null && this.IgnoreColumns.Count != 0)
            {
            ...
            if (this.AppendNavInfo?.AppendProperties?.Count > 0)
            {
                if (result == null)
                {
                    result = "*";
        ...
        {
            if (this.AsTables?.Any(it => it.Key == entityName) == true)
            {
