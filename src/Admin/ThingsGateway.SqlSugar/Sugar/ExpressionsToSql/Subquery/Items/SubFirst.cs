...
namespace ThingsGateway.SqlSugar
{
    public class SubFirst : ISubOperation
    {
        ...
        public string Name
        {
            get
            {
                return nameof(QueryMethodInfo.First);
            }
        }
        ...
        {
            if (exp.Arguments.Count != 0 && exp.Arguments[0] is LambdaExpression)
            {
        ...
        private string GetValueByAuto(MethodCallExpression exp)
        {
            ...
            if (newMemExp == null) { throw new SqlSugarLangException($"Subquery ToList(exp,true) expression {exp.ToString()} can only be it=>new class(){{Id = it.id}}", $"子查询ToList(exp,true)表达式{exp.ToString()}只能是it=>new class(){{ id=it.Id}}"); }
        ...
        {
            return exp.Arguments.Count == 0 && exp.Method.GetGenericArguments().Length == 1;
        }
    }
}
