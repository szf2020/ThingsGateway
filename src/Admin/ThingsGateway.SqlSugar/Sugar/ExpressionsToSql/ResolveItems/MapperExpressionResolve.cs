...
namespace ThingsGateway.SqlSugar
{
    public class MapperExpressionResolve
    {
        ...
        private string GetWhereExpression(MethodCallExpression methodCallExpression)
        {
            ...
            if (querybuiler.Parameters?.Count > 0)
            {
        ...
        void Error01()
        {
            if (mappers == null) { throw new SqlSugarException(ErrorMessage.GetThrowMessage(expression.ToString() + "no support Check if the navigation is configured correctly or Includes() is missing", "当前表达式" + expression.ToString() + " 不支持，查看导航是否配置正确等或者缺少Includes() ")); }
        }
        void ThrowTrue(bool isError)
        {
            if (isError) { throw new SqlSugarException(ErrorMessage.GetThrowMessage(expression.ToString() + "no support Check if the navigation is configured correctly or Includes() is missing", "不支持表达式" + expression.ToString() + "  ，查看导航是否配置正确等或者缺少Includes() ")); }
        }
    }
}