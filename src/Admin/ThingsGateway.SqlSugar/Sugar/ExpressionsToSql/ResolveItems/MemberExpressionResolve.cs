...
namespace ThingsGateway.SqlSugar
{
    public class MemberExpressionResolve : BaseResolve
    {
        ...
        private void ResolveDateDiff(ExpressionParameter parameter, bool? isLeft, MemberExpression expression)
        {
            ...
            {
                if (expression.Member.Name.Contains(item.Key, StringComparison.CurrentCultureIgnoreCase))
                {
                    dateType = item.Value;
                    break;
                }
        ...
        {
            if (parameter.BaseExpression is BinaryExpression || (parameter.BaseParameter.CommonTempData?.Equals(CommonTempDataType.Append) == true))
            {
