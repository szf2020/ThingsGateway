    ...
    public partial class MethodCallExpressionResolve : BaseResolve
    {
        private void CusMethod(ExpressionParameter parameter, MethodCallExpression express, bool? isLeft)
        {
            try
            {
                ...
                if (parameter.BaseParameter.CommonTempData?.Equals(CommonTempDataType.Result) == true)
                {
        ...
        {
            return methodValue?.ToString().Contains("THEN true  ELSE false END") == true;
        }
        ...
        private void AppendModelByIIFMember(ExpressionParameter parameter, MethodCallExpressionModel model, Expression item)
        {
            ...
            if (methodCallExpressionArgs.IsMember && parameter.ChildExpression?.ToString() == "DateTime.Now")
            {
        ...
        private void GetConfigValue(MethodCallExpression express, ExpressionParameter parameter)
        {
            ...
            if (entityDb.Parameter != null && entityDb.Parameter.Count != 0)
            {
        ...
        private object GetMethodValue(string name, MethodCallExpressionModel model)
        {
            ...
            else
            {
                ...
                if (name == "Parse" && TempParseType.IsIn(UtilConstants.GuidType) && model.Args?.Count > 1)
                {
                    name = "Equals";
                }
                else if (name == "Parse")
                ...
                switch (name)
                {
                    ...
                    case "MappingColumn":
                        ...
                        if (string.IsNullOrEmpty(mappingColumnResult))
                        {
        ...
        {
            return SubTools.SubItemsConst.Any(it => it.Name == methodName) && (express.Object?.Type.Name.StartsWith("Subqueryable`") == true);
        }
        ...
        private bool IsDateItemValue(Expression item)
        {
            ...
            if (item is MemberExpression)
            {
                ...
                if (memberExp?.Expression != null && memberExp.Expression.Type == UtilConstants.DateType)
                {
