...
namespace ThingsGateway.SqlSugar
{
    ...
    public partial class BaseResolve
    {
        public string GetNewExpressionValue(Expression item)
        {
            ...
            else if (newContext.SingleTableNameSubqueryShortName != null && newContext.Result?.Contains(this.Context.SqlTranslationLeft + newContext.SingleTableNameSubqueryShortName + this.Context.SqlTranslationRight) == true)
            {
                this.Context.SingleTableNameSubqueryShortName = newContext.SingleTableNameSubqueryShortName;
            }
            ...
            if (item is ParameterExpression)
            {
                ...
                if (this.Context?.SugarContext?.Context != null)
                {
                    var entityInfoColumns = this.Context?.SugarContext?.Context.EntityMaintenance.GetEntityInfo(item.Type)
                        .Columns
                        .Where(it => it.PropertyInfo != ignoreProperty && it.IsIgnore == true && it.PropertyInfo.PropertyType.IsClass());
