    ...
    public static class ExpressionTool
    {
        ...
        internal static string ResolveMemberValue(ExpressionContext context, Expression item, string value)
        {
            if (item is MemberExpression member)
            {
                ...
                {
                    if (value?.Contains('(') == true && !value.Contains(' '))
                    {
        ...
        internal static SugarParameter GetParameterBySqlParameterDbType(int index, object value, ExpressionContext context, Expression member)
        {
            ...
            else
            {
                ...
                if (columnInfo.SqlParameterSize?.ObjToInt() > 0)
                {
...
#pragma warning restore CA1863 // 使用 "CompositeFormat"
        ...
        {
            var result = false;
            while (memberExpr?.Expression != null)
            {
        ...
        public static object DynamicInvoke(Expression expression, MemberExpression memberExpression = null)
        {
            try
            {
                ...
                if (value?.GetType().IsClass() == true && value.GetType() != UtilConstants.StringType && memberExpression != null)
                {
        ...
        internal static bool IsSubQuery(Expression it)
        {
            if (it is MethodCallExpression)
            {
                ...
                if (method.Object?.Type.Name.StartsWith("Subquery") == true)
                {
                    return true;
                }
            }
