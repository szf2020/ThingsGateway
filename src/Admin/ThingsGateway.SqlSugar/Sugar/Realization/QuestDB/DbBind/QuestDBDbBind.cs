namespace ThingsGateway.SqlSugar
{
    public class QuestDBDbBind : DbBindProvider
    {
        public override string GetDbTypeName(string csharpTypeName)
        {
            ...
            if (csharpTypeName == nameof(Single))
            {
                result = "double";
            }
            return result;
        ...
        public override string GetPropertyTypeName(string dbTypeName)
        {
            ...
            else if (key == null)
            {
                if (dbTypeName.StartsWith('_'))
                {
                    ...
                    return MappingTypes.Where(it => it.Value.ToString().Equals(dbTypeName2, StringComparison.CurrentCultureIgnoreCase) || it.Key.Equals(dbTypeName2, StringComparison.CurrentCultureIgnoreCase)).Select(it => it.Value + "[]").First();
                }
