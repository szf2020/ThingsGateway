...
WHERE EVENT_OBJECT_TABLE = '" + tableName + "'");
        ...
        public override bool AddColumn(string tableName, DbColumnInfo columnInfo)
        {
            ...
            if (sql != null && columnInfo.ColumnDescription.HasValue() && !sql.Contains("comment", StringComparison.CurrentCultureIgnoreCase))
            {
        ...
        {
            if (Context.CurrentConnectionConfig.ConnectionString.ObjToString().Contains(charset, StringComparison.CurrentCultureIgnoreCase))
            {
                return true;
            }
            else
