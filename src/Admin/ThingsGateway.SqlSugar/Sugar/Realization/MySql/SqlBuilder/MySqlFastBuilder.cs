...
namespace ThingsGateway.SqlSugar
{
    public partial class MySqlFastBuilder : FastBuilder, IFastBuilder
    {
        ...
        public async Task<int> ExecuteBulkCopyAsync(DataTable dt)
        {
            ...
            {
                return await MySqlConnectorBulkCopy(dt).ConfigureAwait(false);
            }
        ...
        public override async Task CreateTempAsync<T>(DataTable dt)
        {
            ...
            {
                await CreateTempTableForTiDB(dt, tableName).ConfigureAwait(false);
            }
            else
            {
                // 原有的MySQL逻辑
                ...
                await this.Context.Ado.ExecuteCommandAsync($"Create TEMPORARY  table {dt.TableName}({sql}) ").ConfigureAwait(false);
            }
        }
        ...
        private async Task CreateTempTableForTiDB(DataTable dt, string originalTableName)
        {
            ...
            await this.Context.Ado.ExecuteCommandAsync(createTableSql).ConfigureAwait(false);
        }
