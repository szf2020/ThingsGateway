                ...
                foreach (var columns in groupList.Skip((pageIndex - 1) * pageSize).Take(pageSize))
                {
                    ...
                    updateTable.Append("\r\n (" + string.Join(",", columns.Select(it =>
                    {
                        ...
                        if (dbType == null)
                        {
                            ...
                            var isAnyType = PostgreSQLDbBind.MappingTypesConst.Where(x => x.Value.ToString().Equals(typeName, StringComparison.CurrentCultureIgnoreCase)).Any();
                            if (isAnyType)
                            {
                                dbType = PostgreSQLDbBind.MappingTypesConst.Where(x => x.Value.ToString().Equals(typeName, StringComparison.CurrentCultureIgnoreCase)).FirstOrDefault().Key;
                            }
                            else
                            {
                                dbType = "varchar";
