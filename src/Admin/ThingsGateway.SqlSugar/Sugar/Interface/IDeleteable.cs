...
namespace ThingsGateway.SqlSugar
{
    public interface IDeleteable<T> where T : class, new()
    {
        ...
        IDeleteable<T> In<PkType>(IReadOnlyList<PkType> primaryKeyValues);
        IDeleteable<T> InT<PkType>(Expression<Func<T, object>> inField, PkType primaryKeyValue);
        IDeleteable<T> In<PkType>(Expression<Func<T, object>> inField, IReadOnlyList<PkType> primaryKeyValues);
        IDeleteable<T> In<PkType>(Expression<Func<T, object>> inField, ISugarQueryable<PkType> childQueryExpression);

        IDeleteable<T> In<PkType>(string inField, IReadOnlyList<PkType> primaryKeyValues);
        ...
        IDeleteable<T> Where(string whereString, IReadOnlyList<SugarParameter> parameters);
