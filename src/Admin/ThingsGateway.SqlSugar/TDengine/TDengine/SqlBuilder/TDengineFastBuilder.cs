...
namespace SqlSugar.TDengine
{
    public class TDengineFastBuilder : FastBuilder, IFastBuilder
    {
        ...
        public async Task<int> ExecuteBulkCopyAsync(DataTable dt)
        {
            ...
            if (db.TempItems?.ContainsKey(TagKey) == true)
            {
                tagNames = db.TempItems[TagKey] as string[];
            }
            await BulkInsertToTDengine((TDengineConnection)Context.Ado.Connection, dt.TableName, dt, Context.Ado.IsNoTran(), tagNames).ConfigureAwait(false);
            ...
            try
            {
                ...
                if (tagColumns?.Length > 0)
                {
                    ...
                    foreach (var group in groups)
                    {
                        ...
                        var sql = sb.ToString();
                        var result = await Context.Ado.ExecuteCommandAsync(sql).ConfigureAwait(false);
                        sb.Clear();
                    }
                }
                else
                {
                    ...
                    await Context.Ado.ExecuteCommandAsync(insertSql).ConfigureAwait(false);
                }
            }
            catch (Exception ex)
            {
