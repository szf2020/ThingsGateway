...
using AppResource = ThingsGateway.Gateway.Application.AppResource;
...
/// 驱动插件服务
/// </summary>
internal sealed class ManagementPluginService : IPluginService
{
    /// <summary>
    /// 插件驱动文件夹名称
    /// </summary>
    ...
    private const string CacheKeyGetPluginOutputs = $"ThingsGateway.Gateway.Application.{nameof(ManagementPluginService)}GetList";
    ...
    private readonly WaitLock _locker = new(nameof(ManagementPluginService));
    private readonly ILogger _logger;

    public ManagementPluginService(ILogger<ManagementPluginService> logger, IDispatchService<PluginInfo> dispatchService)
    {
        _logger = logger;

        _dispatchService = dispatchService;
        //创建插件文件夹
        Directory.CreateDirectory(PathHelper.CombinePathReplace(AppContext.BaseDirectory, ManagementPluginService.DirName));
        //主程序上下文驱动类字典
    ...
    public List<DriverMethodInfo> GetDriverMethodInfos(string pluginName, IDriver? driver = null)
    {
        ...
        {
            string cacheKey = $"{nameof(ManagementPluginService)}_{nameof(GetDriverMethodInfos)}_{System.Globalization.CultureInfo.CurrentUICulture.Name}";
    ...
    public (IEnumerable<IEditorItem> EditorItems, object Model, Type PropertyUIType) GetDriverPropertyTypes(string pluginName, IDriver? driver = null)
    {
        ...
        {
            string cacheKey = $"{nameof(ManagementPluginService)}_{nameof(GetDriverPropertyTypes)}_{System.Globalization.CultureInfo.CurrentUICulture.Name}";
    ...
    public (IEnumerable<IEditorItem> EditorItems, object Model, Type VariablePropertyUIType) GetVariablePropertyTypes(string pluginName, BusinessBase? businessBase = null)
    {
        ...
        {
            string cacheKey = $"{nameof(ManagementPluginService)}_{nameof(GetVariablePropertyTypes)}_{System.Globalization.CultureInfo.CurrentUICulture.Name}";
    ...
    private WaitLock SaveLock = new(nameof(ManagementPluginService));
    /// <summary>
    /// 异步保存驱动程序信息。
    /// </summary>
    ...
    public void SetDriverProperties(object properties, Dictionary<string, string> deviceProperties)
    {
        // 获取插件的属性信息列表
        var pluginProperties = properties?.GetType().GetRuntimeProperties()
        ...
        foreach (var propertyInfo in pluginProperties ?? new List<PropertyInfo>())
        {
            ...
            // 设置插件属性的值
            propertyInfo.SetValue(properties, value);
        }
    }
    ...
    private void ClearCache()
    {

        {
            ...
            App.CacheService.DelByPattern($"{nameof(ManagementPluginService)}_");
